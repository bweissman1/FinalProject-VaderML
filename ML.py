...
def url_to_sentiment(url):
    global user_list_inp
    url ='https://www.cnn.com/2018/05/04/politics/michael-cohen-donald-trump-campaign/index.html'
    ...
    #scrapes 'my_list' for the values that correspond to each of the sentiments
    #and sorts them into their respective lists.
    for g in range (0,len(user_list_inp)):
        for h in range(0,len(user_list_inp[g])):
            if (user_list_inp[g][h] == ".") and (user_list_inp[g][h-5:h-1] == "neg:"):
                neg.append(user_list_inp[g][h-1:h+3])
            if (user_list_inp[g][h] == ".") and (user_list_inp[g][h-5:h-1] == "neu:"):
                neu.append(user_list_inp[g][h-1:h+3])
            if (user_list_inp[g][h] == ".") and (user_list_inp[g][h-5:h-1] == "pos:"):
                pos.append(user_list_inp[g][h-1:h+3])
            if (user_list_inp[g][h] == ".") and (user_list_inp[g][h-5:h-1] == "und:"):
                comp.append(user_list_inp[g][h-1:h+3])
            if (user_list_inp[g][h-2] == '-'):
                comp.append(user_list_inp[g][h-2:h+3])
    ...
    z = neg
    my_list = [x,y,z,comp]
    print (str(user_list_inp))
    return user_list_inp
...
def plot_whole_dataframe(*args):
    ...
    global user_list_inp
    ax.scatter(args[0][0], args[0][1], args[0][2], c='BLACK')#'RED')
    ax.scatter(args[1][0], args[1][1], args[1][2], c='BLACK')
    ax.scatter(args[2][0], args[2][1], args[2][2], c='BLACK')
    ax.scatter(args[3][0], args[3][1], args[3][2], c='BLACK')
    ax.scatter(args[4][0], args[4][1], args[4][2], c='BLACK')
    ax.scatter(args[5][0], args[5][1], args[5][2], c='BLACK')
    ax.scatter(args[6][0], args[6][1], args[6][2], c='BLACK')
    ax.scatter(args[7][0], args[7][1], args[7][2], c='BLACK')
    ax.scatter(args[8][0], args[8][1], args[8][2], c='BLACK')
    ax.scatter(args[9][0], args[9][1], args[9][2], c='BLACK')
    #ax.scatter(user_list_inp[0],user_list_inp[1],user_list_inp[2],c='YELLOW')
...
def print_values():
    ...
    #k = SourceFrameToListOfLists (dataframe_to_sourceframe(url_to_sentiment)).populate_and_return_lists_of_values()
